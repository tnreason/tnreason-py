...
if __name__ == "__main__":
    ...
    for i in range(shapeDict["b"]):
        assert abs(checkContraction.values[i] - inferer.meanParamDict["c_mesTo_b"].values[i]) < 1e-5
    ...
    for i in range(shapeDict["a"]):
        assert abs(checkContraction.values[i] - inferer.meanParamDict["a_mesTo_b"].values[i]) < 1e-5
