...
sampleNum = 100
samples = application.InferenceProvider(generatingModel).draw_samples(sampleNum, dfOutput=True)

negativeCores = {**representation.create_formulas_cores(currentModel.weightedFormulas),
                 **representation.create_architecture(architecture, headNeuronNames=["n1"])}
...
print(representation.create_solution_expression(architecture, {key.split("_tbo")[0] : maximizer.max_assignment[key] for key in maximizer.max_assignment}))
...
print(representation.create_solution_expression(architecture, {key : assignment[key] for key in assignment}))
