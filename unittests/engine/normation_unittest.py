...
class NormationTest(unittest.TestCase):
    def test_hard_head(self):
        for method in methodList:
            cores = ftc.create_cores_to_expressionsDict({"f1": ["and", "a", "b", 1.23426]},
                                                        coreType=method["coreType"])
            normed = engine.normalize(cores, outColors=["(and_a_b)" + representation.suf.comVarSuf],
    ...
    def test_empty_normation(self):
        for method in methodList:
            normed = engine.normalize({}, outColors=["madeUp1", "madeUp2"], inColors=["madeUp3"],
