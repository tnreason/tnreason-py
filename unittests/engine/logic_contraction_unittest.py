...
class TensorLogicTest(unittest.TestCase):
    ...
    def test_imp(self):
        ...
        for method in methodList:
            cores = ftc.create_cores_to_expressionsDict({"a": ["imp", "a", "b"]}, coreType=method["coreType"])
            contractionResult = engine.contract(coreDict=cores, openColors=["a" + aSuf,
                                                                            "b" + aSuf],
                                                contractionMethod=method["contractionMethod"])
    ...
    def test_eq(self):
        ...
        for method in methodList:
            cores = ftc.create_cores_to_expressionsDict({"a": ["and", ["eq", "a", "b"], ["not", "c"]]},
                                                        coreType=method["coreType"])
            contractionResult = engine.contract(coreDict=cores, openColors=["a" + aSuf,
                                                                            "b" + aSuf],
                                                contractionMethod=method["contractionMethod"])
    ...
    def test_xor(self):
        ...
        for method in methodList:
            cores = ftc.create_cores_to_expressionsDict({"xor": ["and", "c1", ["xor", "a", "b"]]},
                                                        coreType=method["coreType"])
            contractionResult = engine.contract(coreDict=cores, openColors=["a" + aSuf,
                                                                            "b" + aSuf],
                                                contractionMethod=method["contractionMethod"])
    ...
    def test_disconnected_and(self):
        ...
        for method in methodList:
            cores0 = ftc.create_cores_to_expressionsDict({"xor": ["and", "a", "b"]}, coreType=method["coreType"])
            cores1 = ftc.create_cores_to_expressionsDict({"f1": ["and", "a", ["not", "c_2"]],
