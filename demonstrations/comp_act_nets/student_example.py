...
from demonstrations.comp_act_nets.algorithms import contraction_propagation as cp
propagator = cp.ContractionPropagation(studentTensorNetwork, startEdges=["t1"])
propagator.tree_belief_propagation()

## Test on the marginals of the variables "L","G" (core "t1")

testContraction = engine.contract(studentTensorNetwork, openColors=["L","G"])
propContraction = engine.contract({"mes_t0_t1": propagator.messageDict["t1"]["t0"], "t1" : studentTensorNetwork["t1"]}, openColors=["L","G"])
